...
### **AutoCraft (自动仓库搜索)** 🔍
- **功能**: 自动搜索GitHub上包含指定关键词文件且星标数大于指定值的仓库。
- **特点**:
    - **智能搜索**: 基于关键词、星标数、编程语言等条件搜索GitHub仓库
    - **自动化**: 支持定时自动搜索，可设置搜索间隔
    - **灵活配置**: 可自定义搜索关键词、最小星标数、编程语言筛选
    - **结果管理**: 可设置最大结果数、显示详细信息、复制到剪贴板
    - **分支过滤**: 可选择是否包含分支（Fork）仓库
    - **实时状态**: 显示搜索进度和找到的仓库数量
    - **网络优化**: 使用GitHub API进行高效搜索，支持错误处理和重试
    - **用户友好**: 中文界面，详细的搜索结果展示
