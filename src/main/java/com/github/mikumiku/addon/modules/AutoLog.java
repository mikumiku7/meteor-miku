...
public class AutoLog extends BaseModule {
    ...
    private final SettingGroup sgLogging = settings.createGroup("日志设置");
    ...
    private final Setting<Boolean> totemCheck = sgGeneral.add(new BoolSetting.Builder()
        .name("图腾检查")
        .description("启用图腾数量检查，当图腾数量低于设定值时断开连接。")
        .defaultValue(false)
        .build()
    );
    ...
    private final Setting<Boolean> logDisconnectReason = sgLogging.add(new BoolSetting.Builder()
        .name("记录断开原因")
        .description("记录断开连接的详细原因")
        .defaultValue(true)
        .build()
    );

    private final Setting<Boolean> logNearbyPlayers = sgLogging.add(new BoolSetting.Builder()
        .name("记录视野内玩家")
        .description("断开连接时报告视野内的所有玩家")
        .defaultValue(true)
        .build()
    );

    private final Setting<Boolean> logCoordinates = sgLogging.add(new BoolSetting.Builder()
        .name("记录坐标")
        .description("断开连接时记录当前坐标")
        .defaultValue(true)
        .build()
    );
    ...
    public AutoLog() {
        super(BaseModule.CATEGORY_MIKU_COMBAT, "智能Log", "当满足特定条件时自动断开连接。log界面报告视野内玩家和log坐标");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (totemCheck.get()) {
            ...
            if (totems <= totemCount.get()) {
                disconnect("图腾数量低于 " + totemCount.get() + " (当前: " + totems + ")。");
    ...
    private void disconnect(Text reason) {
        MutableText disconnectText = Text.literal("[自动登出] ");
        disconnectText.append(reason);

        // 生成详细的日志信息
        MutableText detailedLog = generateDetailedLog(reason);

        // 如果启用聊天显示日志
        if (logToChat.get()) {
            info("=== 自动登出详细信息 ===");
            info("断开原因: " + reason.getString());

            if (logCoordinates.get()) {
                BlockPos pos = mc.player.getBlockPos();
                info("坐标: X=" + pos.getX() + ", Y=" + pos.getY() + ", Z=" + pos.getZ());
            }
            ...
            if (logNearbyPlayers.get()) {
                List<String> nearbyPlayers = getNearbyPlayersInfo();
                if (!nearbyPlayers.isEmpty()) {
                    info("视野内玩家 (" + nearbyPlayers.size() + "):");
        ...
        // 将详细信息添加到断开连接消息中
        if (logDisconnectReason.get()) {
            disconnectText.append(Text.literal("\n\n=== 详细信息 ===").withColor(Colors.YELLOW));

            if (logCoordinates.get()) {
                BlockPos pos = mc.player.getBlockPos();
                disconnectText.append(Text.literal("\n坐标: " + pos.getX() + ", " + pos.getY() + ", " + pos.getZ()).withColor(Colors.WHITE));
            }

            if (totemCheck.get()) {
                int totems = mc.player.getInventory().count(Items.TOTEM_OF_UNDYING);
                disconnectText.append(Text.literal("\n图腾数量: " + totems).withColor(Colors.WHITE));
            }

            if (logNearbyPlayers.get()) {
                List<String> nearbyPlayers = getNearbyPlayersInfo();
                if (!nearbyPlayers.isEmpty()) {
                    disconnectText.append(Text.literal("\n视野内玩家 (" + nearbyPlayers.size() + "):").withColor(Colors.WHITE));
                    for (String playerInfo : nearbyPlayers) {
                        disconnectText.append(Text.literal("\n  " + playerInfo).withColor(Colors.GRAY));
                    }
                } else {
                    disconnectText.append(Text.literal("\n视野内无其他玩家").withColor(Colors.GRAY));
                }
            }
        }
        ...
        mc.player.networkHandler.onDisconnect(new DisconnectS2CPacket(disconnectText));
    }

    /**
     * 获取视野内玩家信息
     */
    private List<String> getNearbyPlayersInfo() {
        List<String> playerInfoList = new ArrayList<>();

        if (mc.world == null || mc.player == null) return playerInfoList;

        for (PlayerEntity player : mc.world.getPlayers()) {
            if (player.getUuid() != mc.player.getUuid()) {
                double distance = Math.sqrt(mc.player.squaredDistanceTo(player));
                String playerInfo = player.getName().getString() +
                    " (距离: " + String.format("%.1f", distance) + "格, " +
                    "生命值: " + String.format("%.1f", player.getHealth()) + "/" +
                    player.getMaxHealth() + ")";
     ...
     * 生成详细的日志信息
     */
    private MutableText generateDetailedLog(Text reason) {
        MutableText log = Text.literal("自动登出日志:\n");
        log.append(reason);

        if (logCoordinates.get()) {
            BlockPos pos = mc.player.getBlockPos();
            log.append(Text.literal("\n坐标: " + pos.getX() + ", " + pos.getY() + ", " + pos.getZ()).withColor(Colors.WHITE));
        }
