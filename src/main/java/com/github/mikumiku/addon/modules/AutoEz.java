...
public class AutoEz extends BaseModule {
    public AutoEz() {
        super(BaseModule.CATEGORY_MIKU_COMBAT, "自动EZ", "敌人死亡后发送EZ嘲讽消息。");
    }
            ...
            if (packet.getStatus() == 35) {
                ...
                if (pop.get() && mc.player != null && mc.world != null && entity instanceof PlayerEntity) {
                    if (entity != mc.player && !Friends.get().isFriend((PlayerEntity) entity) &&
                        Via.getEntityPos(mc.player).distanceTo(Via.getEntityPos(entity)) <= range.get()) {
    ...
    private boolean anyDead(double range) {
        for (PlayerEntity pl : mc.world.getPlayers()) {
            if (pl != mc.player && !Friends.get().isFriend(pl) && Via.getEntityPos(pl).distanceTo(Via.getEntityPos(mc.player)) <= range
                && pl.getHealth() <= 0) {
