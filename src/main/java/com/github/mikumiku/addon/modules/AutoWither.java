...
public class AutoWither extends BaseModule {
    ...
    public AutoWither() {
        super(CATEGORY_MIKU_BUILD,"自动放凋零", "自动建造凋零");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        if (wither == null) {
            ...
            BlockIterator.register(horizontalRadius.get(), verticalRadius.get(), (blockPos, blockState) -> {
                Direction dir = Via.getOppositeDirectionTo(blockPos);
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        // 灵魂沙/土和头颅槽位
        int slotSand = BagUtil.findItemInventorySlot(Items.SOUL_SAND);
        int slotSoil = BagUtil.findItemInventorySlot(Items.SOUL_SOIL);
        int slotHead = BagUtil.findItemInventorySlot(Items.WITHER_SKELETON_SKULL);

        slotSand = slotSand == -1 ? slotSoil : slotSand;

        if (slotSand == -1 && slotSoil == -1 || slotHead == -1) {
            error("背包中没有足够的资源");
            toggle();
            return;
        }
        ...
        if (blockDelay.get() == 0) {
            // 一 tick 内完成所有
            // 身体
            placeBlock(wither.foot, slotSand);
            placeBlock(wither.foot.up(), slotSand);
            placeBlock(wither.foot.up().offset(wither.axis, -1), slotSand);
            placeBlock(wither.foot.up().offset(wither.axis, 1), slotSand);

            placeBlock(wither.foot.up().up(), slotHead);
            placeBlock(wither.foot.up().up().offset(wither.axis, -1), slotHead);
            placeBlock(wither.foot.up().up().offset(wither.axis, 1), slotHead);
