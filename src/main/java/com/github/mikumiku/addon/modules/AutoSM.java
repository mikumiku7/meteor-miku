    ...
    private PlayerEntity getClosest() {
        ...
        if (!mc.world.getPlayers().isEmpty()) {
            for (PlayerEntity player : mc.world.getPlayers()) {
                if (player != mc.player && (!ignoreFriends.get() || !Friends.get().isFriend(player))) {
                    if (closest == null || Via.getEntityPos(mc.player).distanceTo(Via.getEntityPos(player)) < distance) {
                        closest = player;
                        distance = (float) Via.getEntityPos(mc.player).distanceTo(Via.getEntityPos(player));
                    }
                }
            }
        }
