...
public class ElytraFlyPlus extends BaseModule {
    ...
    private void constantiamTick(PlayerMoveEvent event) {
        ...
        if (motion != null) {
            Via.setMovement(((IVec3d) event.movement), motion.getX(), motion.getY(), motion.getZ());
            event.movement = motion;
        }
    }

    private Vec3d getMotion(Vec3d velocity) {
        if (Via.movementForward(mc.player.input) == 0.0F) {
            return constStop.get() ? new Vec3d(0.0, 0.0, 0.0) : null;
        } else {
            boolean forward = Via.movementForward(mc.player.input) > 0.0F;
    ...
    private void waspTick(PlayerMoveEvent event) {
        if (Via.isFallFlying(mc)) {
            ...
            Via.setMovement(((IVec3d) event.movement), x, y, z);
    ...
    private void updateWaspMovement() {
        ...
        float f = Via.movementForward(mc.player.input);
        float s = Via.movementSideways(mc.player.input);
        if (f > 0.0F) {
            moving = true;
