...
public class AnchorAuraPlus extends BaseModule {
    public AnchorAuraPlus() {
        super(BaseModule.CATEGORY_MIKU_COMBAT, "锚爆光环+", "使用重生锚自动消灭敌人。");
    }
...
//    );
    ...
    private BlockPos posPlace = null;
    ...
    private void onTick(TickEvent.Post event) {
        ...
        blocks = getBlocks(mc.player.getEyePos(), range.get());
    ...
    private void calculate(int index) {
        ...
        for (int i = lastIndex; i < index; i++) {
            ...
            dmg = getDmg(pos);
            self = MikuDamageUtils.anchorDamage(mc.player, mc.player.getBoundingBox(), pos.toCenterPos(), pos, false);

            if (!dmgCheck(dmg, self)) {
                continue;
            }

            Direction direction = BaritoneUtil.getPlaceDirection(pos, true);
            if (direction == null) {
                continue;
            }
            posPlace = pos.offset(direction);
...
//            calcData = data;
        ...
        for (int x = -i; x <= i; x++) {
            for (int y = -i; y <= i; y++) {
                for (int z = -i; z <= i; z++) {
                    ...
                    if (!BaritoneUtil.canPlace(pos) && !(mc.world.getBlockState(pos).getBlock() == Blocks.RESPAWN_ANCHOR)) {
                        continue;
                    }
    ...
    private boolean inRangeToTargets(BlockPos pos) {
        for (PlayerEntity target : targets) {
            if (Via.getEntityPos(target).add(0, 1, 0).distanceTo(Vec3d.ofCenter(pos)) < 3.5) return true;
        }
        return false;
    }

    private void update() {
        if (placePos == null || posPlace == null) return;
    ...
    private void place(Hand hand) {
        BaritoneUtil.placeBlock(posPlace);

        if (placeSwing.get()) {
            BaritoneUtil.swingHand(hand, BaritoneUtil.SwingSide.Client);
        }
    }
    ...
    private boolean placeUpdate() {
        ...
        if (hand == null) {
            switch (switchMode.get()) {
                case Silent -> BagUtil.swapBack();
                case PickSilent -> BagUtil.swapBack();
                case InvSwitch -> BagUtil.swapBack();
            }
        }
        return true;
    }
    ...
    private boolean chargeUpdate(BlockPos pos) {
        ...
        BaritoneUtil.clickBlock(pos, Direction.UP, true, Hand.MAIN_HAND, BaritoneUtil.SwingSide.Client);
    ...
    private boolean explodeUpdate(BlockPos pos) {
        Hand hand = null;
        Direction dir = BaritoneUtil.getPlaceOnDirection(pos);
...
//        interactBlock(hand, pos.toCenterPos(), dir, pos);
        BaritoneUtil.clickBlock(pos, dir, true, hand, BaritoneUtil.SwingSide.All);
    ...
    private double getDmg(BlockPos pos) {
        double highest = -1;
        for (PlayerEntity target : targets) {
            highest = Math.max(highest, MikuDamageUtils.anchorDamage(target, target.getBoundingBox(), pos.toCenterPos(), pos, true));
        }
        return highest;
    }
