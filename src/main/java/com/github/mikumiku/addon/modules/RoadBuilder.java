...
public class RoadBuilder extends BaseModule {
    ...
    private void tryPlace(BlockPos pos) {
        ...
        int slotResult = BagUtil.findItemInventorySlotGrim(blockType.get().asItem());
        ...
        placeList.add(pos);
        BagUtil.doSwap(slotResult);

        // 根据方块类型和 slapType 设置选择放置方式
        boolean placed = false;
        if (blockType.get() instanceof SlabBlock) {
            if (slapType.get() == SlapType.UP) {
                placed = BaritoneUtil.placeUpBlock(pos, true, true, true);
            } else {
                placed = BaritoneUtil.placeDownBlock(pos, true, true, true);
            }
        } else {
            placed = BaritoneUtil.placeBlock(pos);
        }
