...
public class AutoUseItems extends BaseModule {
    ...
    public AutoUseItems() {
        super(BaseModule.CATEGORY, "自动吃喝", "自动使用物品，支持多种触发条件");
    }
...
//        }
    ...
    private boolean shouldContinueUsing() {
        ...
        // 如果刚开始使用（useTimer 还很大），给一些时间让物品开始使用
        if (useTimer > useDelay.get() - 5) return true;
