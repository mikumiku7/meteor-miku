...
public class BaritoneUtil {
     ...
     * 需要潜行才能交互的方块类列表
     */
    public static final List<Class> SNEAK_BLOCK_CLASSES = Arrays.asList(SignBlock.class,
        DoorBlock.class,
        ButtonBlock.class,
        TrapdoorBlock.class,
    ...
    public static boolean canClick(BlockPos pos) {
        return mc.world.getBlockState(pos).isSolid()
            && (!SNEAK_BLOCKS.contains(getBlock(pos)) && !(getBlock(pos) instanceof BedBlock) || mc.player.isSneaking());
    }
    ...
    public static boolean isStrictDirection(BlockPos pos, Direction side) {
        if (mc.player.getBlockY() - pos.getY() >= 0 && side == Direction.DOWN) {
            return false;
        } else if (side == Direction.UP && pos.getY() + 1 > mc.player.getEyePos().getY()) {
            return false;
            ...
            if (canClick(pos.offset(i)) && !canReplace(pos.offset(i)) && canSeeBlockFace(pos.offset(i), i.getOpposite())) {
                double vecDis = mc
                    ...
                    .squaredDistanceTo(
                        pos.toCenterPos()
                            .add(
                                i.getVector().getX() * 0.5,
                                i.getVector().getY() * 0.5,
                                i.getVector().getZ() * 0.5
                            )
                    );
    ...
    public static boolean airPlaceBlock(BlockPos pos) {
        ...
        if (side == null) {
            ...
            neighbor = pos.offset(side.getOpposite());
        }
    ...
    public static boolean placeBlock(BlockPos pos, Item item) {
        ...
        if (mc.player.squaredDistanceTo(pos.getX(), pos.getY(), pos.getZ()) > 36) {
            return false;
        }
    ...
    public static boolean placeBlock(BlockPos pos, Direction direction, boolean clientSwing, boolean rotate) {
        ...
        Vec3d hitVec = pos.toCenterPos().add(new Vec3d(direction.getUnitVector()).multiply(MikuMagic2.hit));
        if (rotate) {
    ...
    public static boolean placeBlockImmediately(BlockHitResult result, boolean clientSwing) {
        ...
        boolean shouldSneak = (SNEAK_BLOCKS.contains(state.getBlock())
            || isSneakBlockClass(mc.world.getBlockState(result.getBlockPos()).getBlock()))
     ...
     * <p>
     * BaritoneUtil.clickBlock(plantPos.get(i), WalkDirection.UP, true, Hand.MAIN_HAND, SwingSide.All);
     *
     * @param pos
     * @param side
     * @param rotate
