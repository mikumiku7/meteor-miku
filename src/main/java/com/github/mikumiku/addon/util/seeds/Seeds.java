...
public class Seeds extends System<Seeds> {
    ...
    public Seeds fromTag(NbtCompound tag) {
        tag.getKeys().forEach(key -> {
            ...
            if (compound instanceof Optional<?> optional) {
                key2 = (NbtCompound) optional.get();
            } else {
                key2 = (NbtCompound) compound;
            }
    ...
    private static void sendInvalidVersionWarning(String seed, String targetVer) {
        MutableText msg = Text.literal(String.format("无法解析 Minecraft 版本 \"%s\". 改用 %s. 如果您想更改版本，请运行: ", targetVer, MCVersion.latest().name));
